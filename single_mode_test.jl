...
if idx_00 > 0
    # Set coefficient for (0,0) mode
    sh_coeffs[idx_00] = 1.0 + 0.0im
    
    println("Original coefficient for (l=0,m=0): ", sh_coeffs[idx_00])
    ...
    println("Reconstructed coefficient (before calibration): ", sh_reconstructed[idx_00])
    println("Ratio (reconstructed/original): ", sh_reconstructed[idx_00] / sh_coeffs[idx_00])
    
    # Now apply full analysis with calibration
    sh_full = SHTnsKit.cplx_spat_to_sh(cfg, spatial)
    println("Reconstructed coefficient (after calibration): ", sh_full[idx_00])
    println("Final ratio (full/original): ", sh_full[idx_00] / sh_coeffs[idx_00])
    
    # Check what the calibration scale factor is
    scale = SHTnsKit._get_complex_scale!(cfg)
    println("Calibration scale factor: ", scale)
else
    println("Could not find (l=0, m=0) mode")
end
